        ...
        >
            <form onSubmit={submit} className="relative z-10 space-y-8">
                <div className="flex justify-between items-start">
                    <h2 className="text-3xl font-black italic tracking-tighter">{step ? __('Edit Step') : __('Add New Step')}</h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                    ...
                    <div className="space-y-4">
                        <label className="text-[10px] font-black uppercase tracking-widest opacity-50 block ml-2">{__('Icon Name')}</label>
                        <input 
                            value={data.icon} 
                            onChange={e => setData('icon', e.target.value)}
                            className="w-full bg-white/10 border-none rounded-2xl p-6 text-white placeholder:text-white/30 font-black italic shadow-inner focus:ring-4 focus:ring-white/10 transition-all text-sm"
                            placeholder="e.g. Target, Search, Shield"
                        />
                    </div>
